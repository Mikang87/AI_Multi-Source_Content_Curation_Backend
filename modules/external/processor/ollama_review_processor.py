...
from app.modules.external.processor.base_llm_processor import BaseLLMProcessor, LLMReviewProcessingResult
...
class OllamaReviewProcessor(BaseLLMProcessor):
    ...
    async def summarize(self, raw_text: str) -> LLMReviewProcessingResult:
        prompt = f"""
        ...
        try:
            ...
            return LLMReviewProcessingResult(
    ...
    async def summarize_and_extract_keywords(self, raw_text: str):
        """부모 클래스의 추상 메서드 구현 (게임 리뷰 프로세서에서는 사용하지 않음)"""
        raise NotImplementedError("Review processor uses 'summarize' method instead.") 